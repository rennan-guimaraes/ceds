---
title: "Terceira Atividade - Validando resultados"
...
Caso esteja queira acessar o código fonte ou a versão em HTML, acesse o [repositório da atividade](https://github.com/rennan-guimaraes/ceds/tree/main/introduction_ds/third_exercise).
...
## Combinando as Métricas de Resample
...
## Calculando a Média e Variância das Métricas
...
# Análise Atualizada dos Modelos de Machine Learning na Base German Credit
...
## Ajuste Final e Avaliação no Conjunto
...
## Compilando as Métricas do Conjunto de Teste e avaliando resultados
...
final_recipe <- recipe(Good_loan ~ ., data = full_train_data) %>%
  # Copie os mesmos passos da receita original
  # Imputação
  step_impute_mode(all_nominal_predictors()) %>%
  step_impute_median(all_numeric_predictors()) %>%
  # Tratamento de outliers usando winsorization
...
```{r}
# Como o professor pediu para ser só um exemplo de deploy, vamos utilizar uma parte dos dados do conjunto que já utilizamos no treinamento, entretanto isso não deve ser feito na prática.
set.seed(17)  # Para reprodutibilidade
new_data <- full_train_data %>%
  sample_n(10) %>%  # Seleciona 10 amostras aleatórias
  select(-Good_loan)  # Remove a variável alvo
...
# Combinar as previsões com os dados originais para melhor visualização
results <- bind_cols(
  new_data,
  predictions
)
...
saveRDS(final_model, "german_credit_final_model.rds")
```
