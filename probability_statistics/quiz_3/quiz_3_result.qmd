...
A Telecom Satélites fornece serviços de comunicação para empresas na área metropolitana. O escritório da companhia acabou de receber a informação (amostra) de que a média de tempo de transmissão via satélite é de 150 segundos, com desvio padrão de 15 segundos. O tempo aparenta ser normalmente distribuído. Para estimular consumidores a solicitarem seus serviços e para estabelecer taxas que proporcionem lucro, a Telecom deve determinar qual a probabilidade que algumas chamadas aconteçam. O diretor de serviços quer que você faça uma estimativa da probabilidade de que uma chamada demore entre os intervalos mostrados nos itens abaixo. Mas antes, você deve avaliar se os dados da amostra que receberam seguem uma distribuição Normal. A amostra de dados é mostrada a seguir:
...
O teste de Kolmogorov-Smirnov compara a distribuição acumulada empírica dos dados com a distribuição normal teórica. O p-valor obtido é usado para testar a hipótese nula de que os dados seguem uma distribuição normal. Se o p-valor for menor que 0,05, rejeitamos a hipótese de normalidade.
...
O teste de Shapiro-Wilk é utilizado para verificar a normalidade dos dados. Um p-valor maior que 0,05 indica que não podemos rejeitar a hipótese de normalidade.
...
O teste de Anderson-Darling é sensível a discrepâncias na cauda da distribuição. Um p-valor maior que 0,05 indica que os dados podem ser considerados normais.
...
O teste de Lilliefors é uma adaptação do teste de Kolmogorov-Smirnov quando os parâmetros da distribuição normal não são conhecidos e precisam ser estimados. Um p-valor maior que 0,05 indica normalidade.

**Conclusão Geral:**

Se em todos os testes o p-valor for maior que 0,05, não rejeitamos a hipótese de que os dados seguem uma distribuição normal. Portanto, podemos considerar os dados como normalmente distribuídos.
...
A probabilidade de uma chamada demorar entre 125 e 150 segundos é aproximadamente `r round(prob_b*100, 2)`%.
...
A probabilidade de uma chamada demorar menos de 125 segundos é aproximadamente `r round(prob_c*100, 2)`%.
...
A probabilidade de uma chamada demorar entre 145 e 155 segundos é aproximadamente `r round(prob_d*100, 2)`%.
...
A probabilidade de uma chamada demorar entre 160 e 165 segundos é aproximadamente `r round(prob_e*100, 2)`%.
...
Analisando os p-valores dos testes de Kolmogorov-Smirnov, escolhemos a distribuição que não rejeita a hipótese nula de aderência aos dados (p-valor maior que 0,05). Supondo que a distribuição Weibull apresenta o melhor ajuste.
...
O resultado deve ser próximo de 1, confirmando que a área sob a curva da distribuição de probabilidade é igual a 1.
...
Se os p-valores forem maiores que 0,05, não rejeitamos a hipótese de normalidade.
...
Baseado nos p-valores, determinar se a série histórica pode ser considerada normalmente distribuída.

---

## Exercício 4: Identificação de Distribuição

Neste exercício, iremos identificar a distribuição adequada para cada conjunto de dados fornecido, utilizando técnicas estatísticas e ferramentas do R.
...
Realizamos o ajuste para as distribuições Weibull, Gamma, Lognormal e Normal.
...
A distribuição com o menor AIC é a que melhor se ajusta aos dados. Observamos que a distribuição Gamma apresenta o menor AIC.
...
O p-valor do teste de Kolmogorov-Smirnov é maior que 0,05, não rejeitando a hipótese nula de que os dados seguem uma distribuição Gamma.
...
A distribuição Weibull apresenta o menor AIC, indicando o melhor ajuste.

#### Teste de Kolmogorov-Smirnov

```{r}
ks.test(dados_b, "pweibull", shape=ajuste_weibull_b$estimate["shape"], scale=ajuste_weibull_b$estimate["scale"])
```

**Interpretação:**

O p-valor é maior que 0,05, não rejeitando a hipótese de aderência à distribuição Weibull.

#### Plotagem do Histograma com a Curva Ajustada

```{r}
hist(dados_b, breaks=10, freq=FALSE, col="lightgreen", main="Ajuste da Distribuição Weibull", xlab="Valores")
curve(dweibull(x, shape=ajuste_weibull_b$estimate["shape"], scale=ajuste_weibull_b$estimate["scale"]), 
...
A distribuição Lognormal apresenta o menor AIC, indicando o melhor ajuste.
...
O p-valor é maior que 0,05, não rejeitando a hipótese de aderência à distribuição Lognormal.
...
A distribuição Normal apresenta o menor AIC, indicando o melhor ajuste.

#### Teste de Kolmogorov-Smirnov

```{r}
ks.test(dados_d, "pnorm", mean=ajuste_normal_d$estimate["mean"], sd=ajuste_normal_d$estimate["sd"])
```

**Interpretação:**

O p-valor é maior que 0,05, não rejeitando a hipótese de aderência à distribuição Normal.

#### Plotagem do Histograma com a Curva Ajustada

```{r}
hist(dados_d, breaks=10, freq=FALSE, col="lightyellow", main="Ajuste da Distribuição Normal", xlab="Valores")
curve(dnorm(x, mean=ajuste_normal_d$estimate["mean"], sd=ajuste_normal_d$estimate["sd"]), 
      col="darkgreen", lwd=2, add=TRUE)
legend("topright", legend="Distribuição Normal", col="darkgreen", lwd=2)
```
...
```{r}
ks.test(dados_e, "pnorm", mean=ajuste_normal_e$estimate["mean"], sd=ajuste_normal_e$estimate["sd"])
```
...
```{r}
hist(dados_e, breaks=10, freq=FALSE, col="lightgray", main="Ajuste da Distribuição Normal", xlab="Valores")
curve(dnorm(x, mean=ajuste_normal_e$estimate["mean"], sd=ajuste_normal_e$estimate["sd"]), 
      col="orange", lwd=2, add=TRUE)
legend("topright", legend="Distribuição Normal", col="orange", lwd=2)
```